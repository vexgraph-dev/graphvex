 ...
 * ============================================================================
 * Gradient/pattern brush in vector space (resolution-free): fixed 8 stops
 * plus a 6-float affine snap. Embeds Brush first so a VectorBrush upcasts
 * to Brush by address. Tint and master alpha come from the embedded base;
 * pipelines resolve interior color from the stops. The struct rides the
 * vexspoke arena via Memory_alloc(TYPE_VECTOR_BRUSH_SINGLETON) with a
...
static void vectorBrushInitStops(VectorBrush *self, uint32_t c0, uint32_t c1) {
    (*self).colors[0] = c0;
    (*self).colors[1] = c1;
    for (uint32_t i = 2; i < VECTOR_BRUSH_STOPS_MAX; i++)
        (*self).colors[i] = 0u;
    (*self).offsets[0] = 0.0f;
    (*self).offsets[1] = 1.0f;
    for (uint32_t i = 2; i < VECTOR_BRUSH_STOPS_MAX; i++)
...
void VectorBrush_setStopCount(VectorBrush *self, uint32_t n) {
    if (!self)
        return;
    if (n > VECTOR_BRUSH_STOPS_MAX)
        n = VECTOR_BRUSH_STOPS_MAX;
    (*self).stopCount = n;
}
...
void VectorBrush_setStop(VectorBrush *self, uint32_t index, uint32_t color, float offset) {
    if (!self)
        return;
    if (index >= VECTOR_BRUSH_STOPS_MAX)
        return;
...
void VectorBrush_setColors(const uint32_t *colors, uint32_t count, VectorBrush *dest) {
    if (!colors || !dest)
        return;
    if (count > VECTOR_BRUSH_STOPS_MAX)
        count = VECTOR_BRUSH_STOPS_MAX;
...
uint32_t VectorBrush_getStopColor(const VectorBrush *self, uint32_t index) {
    if (!self || index >= VECTOR_BRUSH_STOPS_MAX)
        return 0u;
